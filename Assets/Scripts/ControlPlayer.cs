...
public class ControlPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        if (estaMuerto || juegoTerminado) return;
    ...
    public void Saltar()
    {
        if (enSuelo && !estaMuerto && !juegoTerminado)
        {
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (ControlPuntaje.Instancia != null && ControlPuntaje.Instancia.puntaje >= 100)
            {
                Debug.Log("¡Haz ganado! Llegaste a los 100 puntos");
                ...
                StartCoroutine(CambioSegundaEscena(2f));
            }
            else
            {
                Debug.Log("Tienes que llegar a los 100 puntos");
            }
        }
    }
    ...
    IEnumerator CambioSegundaEscena(float segundos)
    {
        yield return new WaitForSeconds(segundos);
